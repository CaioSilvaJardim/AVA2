...
export default function HomePage() {
  ...
  async function handleConnect() {
    ...
    try {
      ...
      // Valida que popupUrl é uma URL real antes de salvar
      const popupUrl = data.popupUrl || data.liveUrl || '';
      if (!popupUrl || popupUrl === 'null' || !popupUrl.startsWith('http')) {
        setError(`URL do browser inválida: "${popupUrl}". Verifique os logs do servidor.`);
        setLoading(false);
        return;
      }

      localStorage.setItem('visitorId', data.visitorId);
      localStorage.setItem('popupUrl', popupUrl);

      router.push('/connect');
    } catch (err) {
      setError(`Falha na conexão: ${err instanceof Error ? err.message : String(err)}`);
      setLoading(false);
    }
  }
            ...
            >
              ...
              {error && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="text-red-400 text-xs border border-red-900 px-3 py-2 break-all"
                >
                  &gt; ERRO: {error}
                </motion.div>
              )}
